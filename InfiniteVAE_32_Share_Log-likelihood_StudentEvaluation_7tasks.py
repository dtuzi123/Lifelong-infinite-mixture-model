...
class LifeLone_MNIST(object):
    ...
    def build_model(self):
        ...
        self.Student_recoloss = student_reconstruction_loss1
    ...
    def Calculate_MSE_ByIndex_Student(self,testX,index):
        p1 = int(np.shape(testX)[0] / self.batch_size)
        myPro = self.Student_recoloss
        sumError = 0
        for i in range(p1):
            g = testX[i * self.batch_size:(i + 1) * self.batch_size]
            sumError = sumError + self.sess.run(myPro, feed_dict={self.inputs: g})
    ...
    def Calculate_SSIM_ByIndex_Student(self,testX,index):
        p1 = int(np.shape(testX)[0] / self.batch_size)
        myReco = self.Student_Reco
        mySum = 0
        for i in range(p1):
            g = testX[i * self.batch_size:(i + 1) * self.batch_size]
            r = self.sess.run(myReco,feed_dict={self.inputs:g})
    ...
    def Calculate_PSNR_ByIndex_Student(self,testX,index):
        p1 = int(np.shape(testX)[0] / self.batch_size)
        myReco = self.Student_Reco
        mySum = 0
        for i in range(p1):
    ...
    def Calculate_Student_ELBO(self,testX,index):
        p1 = int(np.shape(testX)[0] / self.batch_size)
        myReco = self.StudentLoss
        mySum = 0
        for i in range(p1):
            g = testX[i * self.batch_size:(i + 1) * self.batch_size]
            r = self.sess.run(myReco,feed_dict={self.inputs:g})
    ...
    def Calculate_StudentCriteria(self,testX):
        mse = self.Calculate_MSE_ByIndex_Student(testX, 0)
        ssim = self.Calculate_SSIM_ByIndex_Student(testX, 0)
        psnr = self.Calculate_PSNR_ByIndex_Student(testX, 0)
        elbo = self.Calculate_Student_ELBO(testX,0)
        return mse, ssim, psnr, elbo
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            mse1,ssim1,psnr1,elbo1 = self.Calculate_StudentCriteria(self.mnist_test_x)
            mse2,ssim2,psnr2,elbo2 = self.Calculate_StudentCriteria(self.svhn_test_x)
            mse3,ssim3,psnr3,elbo3 = self.Calculate_StudentCriteria(self.FashionTest_x)
            mse4,ssim4,psnr4,elbo4 = self.Calculate_StudentCriteria(self.InverseFashionTest_x)
            mse5,ssim5,psnr5,elbo5 = self.Calculate_StudentCriteria(self.cifar_test_x)
            mse6,ssim6,psnr6,elbo6 = self.Calculate_StudentCriteria(self.zimu[0:10000])

            sum1 = mse1 + mse2 + mse3 + mse4 + mse5 + mse6
            sum1 = sum1 / 6.0

            sum2 = ssim1 + ssim2 + ssim3 + ssim4 + ssim5 + ssim6
            sum2 = sum2 / 6.0

            sum3 = psnr1 + psnr2 + psnr3 + psnr4 + psnr5 + psnr6
            sum3 = sum3 / 6.0

            sum4 = elbo1 + elbo2 + elbo3+ elbo4+ elbo5 + elbo6
            sum4 = sum4 / 6.0

            #mnist, Fashion, Svhn, InverseFashion, InverseMnist
            print(
                "MSE: %4.4f SSMI: %4.4f, PSNR: %.8f ELBO: %.8f" \
                % (mse1, ssim1, psnr1,elbo1))
            print('\n')
            print("MSE: %4.4f SSMI: %4.4f, PSNR: %.8f ELBO: %.8f" \
                  % (mse2, ssim2, psnr2,elbo2))
            print('\n')
            print("MSE: %4.4f SSMI: %4.4f, PSNR: %.8f ELBO: %.8f" \
                  % (mse3, ssim3, psnr3, elbo3))
            print('\n')
            print("MSE: %4.4f SSMI: %4.4f, PSNR: %.8f ELBO: %.8f" \
                  % (mse4, ssim4, psnr4, elbo4))
            print('\n')
            print("MSE: %4.4f SSMI: %4.4f, PSNR: %.8f ELBO: %.8f" \
                  % (mse5, ssim5, psnr5, elbo5))
            print('\n')
            print("MSE: %4.4f SSMI: %4.4f, PSNR: %.8f ELBO: %.8f" \
                  % (mse6, ssim6, psnr6, elbo6))
            print('\n')
            print("MSE: %4.4f SSMI: %4.4f, PSNR: %.8f ELBO: %.8f" \
                  % (sum1, sum2, sum3, sum4))
