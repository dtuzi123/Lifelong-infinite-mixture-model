...
def Image_classifier(inputs, scopename, is_training=True, reuse=False):
    with tf.compat.v1.variable_scope(scopename, reuse=reuse):
...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        self.RFashionTrain_x, self.RFashionTrain_label, self.RFashionTest_x, self.RFashionTest_label = GiveFashion32()
    ...
    def Create_NewExpert(self):
        ...
        z_myStr = "z_encoder" + str(self.componentCount)

        # encoder continoual information
        self.Shared_Encoder = Encoder_SVHN_Shared_Supervised_2(self.inputs,self.z, "encoder_shared", batch_size=64, reuse=True)

        a1 = Generator_SharedSVHN("VAE_Generator_Shared", tf.concat((self.z,self.labels),axis=1), reuse=True)
        G1 = Generator_SubSVHN(myStr2, a1, reuse=False)
        self.VAE_GeneratorList.append(G1)

        # encoder continoual information
        logit,softmaxValue = Encoder_SVHN_Specific_Supervised_2(self.Shared_Encoder, myStr, batch_size=64, reuse=False)
        ...
        Shared_Encoder_z = Encoder_SVHN_Shared(self.inputs, "z_encoder_shared", batch_size=64, reuse=True)
        z_mean1, z_log_sigma_sq1, features = Encoder_SVHN_Specific(Shared_Encoder_z, z_myStr, batch_size=64,
                                                                   reuse=False)
        ...
        vaeloss1 = reconstruction_loss1 + classLoss + KL_divergence1
    ...
    def Activate_Expert(self, index):
        ...
        z_myStr = "z_encoder" + str(index)

        self.Shared_Encoder = Encoder_SVHN_Shared_Supervised_2(self.inputs,self.z, "encoder_shared", batch_size=64, reuse=True)

        # encoder continoual information
        logit, softmaxValue = Encoder_SVHN_Specific_Supervised_2(self.Shared_Encoder, myStr, batch_size=64, reuse=True)
        softmaxValue1 = tf.nn.softmax(logit)
        log_y1 = tf.log(softmaxValue1 + 1e-10)
        discrete1 = my_gumbel_softmax_sample(log_y1, np.arange(10))
        ...
        ELBO = reconstruction_loss1+self.classLoss + KL_divergence1
        self.vaeLoss = ELBO
        ...
        z_Encoder_vars1 = [var for var in T_vars if var.name.startswith(z_myStr)]
        vae_vars = GAN_generator_vars1+Encoder_vars1+z_Encoder_vars1
        with tf.compat.v1.variable_scope("foo", reuse=tf.AUTO_REUSE):
        ...
        self.sess.run(tf.compat.v1.variables_initializer(not_initialized_vars))
    ...
    def build_model(self):
        ...
        z_dim = 150
        # encoder continoual information
        self.Shared_Encoder = Encoder_SVHN_Shared_Supervised_2(self.inputs,self.z, "encoder_shared", batch_size=64, reuse=False)
        logit,softmaxValue = Encoder_SVHN_Specific_Supervised_2(self.Shared_Encoder, "encoder1", batch_size=64, reuse=False)
        softmaxValue1 = tf.nn.softmax(logit)
        log_y1 = tf.log(softmaxValue1 + 1e-10)
        discrete1 = my_gumbel_softmax_sample(log_y1, np.arange(10))
        ...
        vaeloss1 = reconstruction_loss1 + self.classLoss + KL_divergence1
        ...
        z_shared_vars = [var for var in T_vars if var.name.startswith('z_encoder_shared')]
        z_encoder_vars1 = [var for var in T_vars if var.name.startswith('z_encoder1')]

        vae_vars = VAE_encoder_vars + VAE_generator_vars+VAE_generator_Shared_vars+VAE_encoder_Shared_vars+z_encoder_vars1+z_shared_vars
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            self.RFashionTrain_x = tf.image.flip_left_right(self.RFashionTrain_x)
            self.RFashionTest_x = tf.image.flip_left_right(self.RFashionTest_x)
            self.RFashionTrain_x = self.RFashionTrain_x.eval()
            self.RFashionTest_x = self.RFashionTest_x.eval()
            ...
            for taskIndex in range(taskCount):
                ...
                if taskIndex == 0:
                    ...
                    currentTrainY = self.mnist_label
                elif taskIndex == 1:
                    currentTrainX = self.svhn_train_x
                    currentTrainY = self.svhn_label
                elif taskIndex == 2:
                    currentTrainX = self.FashionTrain_x
                    currentTrainY = self.FashionTrain_label
                elif taskIndex == 3:
                    currentTrainX = self.InverseFashionTrain_x
                    currentTrainY = self.InverseFashionTrain_label
                elif taskIndex == 4:
                    currentTrainX = self.InverseMNISTTrain_x
                    currentTrainY = self.InverseMNISTTrain_label
                elif taskIndex == 5:
                    currentTrainX = self.RFashionTrain_x
                    currentTrainY = self.RFashionTrain_label
                elif taskIndex == 6:
                    currentTrainX = self.cifar_train_x
                    currentTrainY = self.cifar_train_label
            ...
            index1 = self.Select_Expert_ByData2(self.mnist_test_x,self.mnist_label_test)
            mnistError = self.Calculate_Accuracy_ByIndex(self.mnist_test_x,self.mnist_label_test, index1)
            print("index:")
            print(index1)

            index3 = self.Select_Expert_ByData2(self.svhn_test_x,self.svhn_label_test)
            svhnError = self.Calculate_Accuracy_ByIndex(self.svhn_test_x,self.svhn_label_test, index3)
            print("index:")
            print(index3)

            index2 = self.Select_Expert_ByData2(self.FashionTest_x,self.FashionTest_label)
            fashionError = self.Calculate_Accuracy_ByIndex(self.FashionTest_x,self.FashionTest_label, index2)
            print("index:")
            print(index2)

            index4 = self.Select_Expert_ByData2(self.InverseFashionTest_x,self.InverseFashionTest_label)
            IFashionError = self.Calculate_Accuracy_ByIndex(self.InverseFashionTest_x,self.InverseFashionTest_label, index4)
            print("index:")
            print(index4)

            index5 = self.Select_Expert_ByData2(self.InverseMNISTTest_x,self.InverseMNISTTest_label)
            IMNISTError = self.Calculate_Accuracy_ByIndex(self.InverseMNISTTest_x,self.InverseMNISTTest_label,index5)
            print("index:")
            print(index5)

            index6 = self.Select_Expert_ByData2(self.RFashionTest_x,self.RFashionTest_label)
            RFashionError = self.Calculate_Accuracy_ByIndex(self.RFashionTest_x,self.RFashionTest_label, index6)
            print("index:")
            print(index6)

            index7 = self.Select_Expert_ByData2(self.cifar_test_x,self.cifar_test_label)
            cifarError = self.Calculate_Accuracy_ByIndex(self.cifar_test_x,self.cifar_test_label, index7)
            print("index:")
            print(index7)

            sum1 = mnistError  + svhnError+fashionError + IFashionError + IMNISTError+ RFashionError+cifarError
            sum1 = sum1 / 7.0
