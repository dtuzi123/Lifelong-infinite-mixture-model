...
class LifeLone_MNIST(object):
    ...
    def Create_NewExpert(self):
        ...
        myStr = "c_encoder" + str(self.componentCount)
        ...
        self.Shared_Encoder = Encoder_SVHN_Shared_Supervised2(self.inputs,"c_encoder_shared", batch_size=64, reuse=True)
        ...
        # encoder continoual information
        logit,softmaxValue = Encoder_SVHN_Specific_Supervised(self.Shared_Encoder, myStr, batch_size=64, reuse=False)
    ...
    def Activate_Expert(self, index):
        ...
        # encoder continoual information
        logit, softmaxValue = Encoder_SVHN_Specific_Supervised(self.Shared_Encoder, myStr, batch_size=64, reuse=True)
        softmaxValue1 = tf.nn.softmax(logit)
        log_y1 = tf.log(softmaxValue1 + 1e-10)
        discrete1 = my_gumbel_softmax_sample(log_y1, np.arange(10))
        ...
        Encoder_vars2 = [var for var in T_vars if var.name.startswith(encoder_myStr)]
        vae_vars = GAN_generator_vars1 + Encoder_vars1 + Encoder_vars2
        with tf.variable_scope("foo", reuse=tf.AUTO_REUSE):
            ...
            self.classifier_optim = tf.train.AdamOptimizer(self.learning_rate, beta1=self.beta1) \
                .minimize(self.classLoss, var_list=Encoder_vars1)
    ...
    def build_model(self):
        ...
        z_dim = 150
        # encoder continoual information
        self.Shared_Encoder = Encoder_SVHN_Shared_Supervised2(self.inputs, "c_encoder_shared", batch_size=64, reuse=False)
        logit,softmaxValue = Encoder_SVHN_Specific_Supervised(self.Shared_Encoder, "c_encoder1", batch_size=64, reuse=False)
        softmaxValue1 = tf.nn.softmax(logit)
        log_y1 = tf.log(softmaxValue1 + 1e-10)
        discrete1 = my_gumbel_softmax_sample(log_y1, np.arange(10))
        ...
        T_vars = tf.trainable_variables()
        VAE_encoder_vars_c = [var for var in T_vars if var.name.startswith('c_encoder1')]
        VAE_encoder_vars_z = [var for var in T_vars if var.name.startswith('z_encoder1')]
        ...
        VAE_encoder_Shared_vars_c = [var for var in T_vars if var.name.startswith('c_encoder_shared')]
        VAE_encoder_Shared_vars_z = [var for var in T_vars if var.name.startswith('z_encoder_shared')]

        vae_vars = VAE_encoder_vars_c+VAE_encoder_vars_z+VAE_generator_vars+VAE_generator_Shared_vars+VAE_encoder_Shared_vars_c+VAE_encoder_Shared_vars_z
        classifier_vars = VAE_encoder_Shared_vars_c+VAE_encoder_vars_c
        ...
        with tf.variable_scope("foo", reuse=tf.AUTO_REUSE):
            self.vae_optim = tf.train.AdamOptimizer(self.learning_rate, beta1=self.beta1) \
                .minimize(self.vaeLoss, var_list=vae_vars)
            self.classifier_optim = tf.train.AdamOptimizer(self.learning_rate, beta1=self.beta1) \
                .minimize(self.classLoss, var_list=classifier_vars)
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            for taskIndex in range(taskCount):
                ...
                for epoch in range(start_epoch, self.epoch):
                    ...
                    for idx in range(start_batch_id, self.num_batches):
                        ...
                        _, vaeLoss,_,class_loss = self.sess.run(
                        ...
                        print(
                            ...
                            % (epoch, idx, self.num_batches, time.time() - start_time, vaeLoss, class_loss, 0, 0))
            ...
            index6 = self.Select_Expert_ByData2(self.cifar_test_x, self.cifar_test_label)
            CIFAR10Error = self.Calculate_Accuracy_ByIndex(self.cifar_test_x, self.cifar_test_label, index6)
            print("index:")
            print(index6)

            sum1 = mnistError + fashionError + svhnError + IFashionError + IMNISTError + CIFAR10Error
            sum1 = sum1 / 6.0
