...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        x_train = datax[0:60000]
        x_test = datax[60000:70000]
        y_train = datay[0:60000]
        y_test = datay[60000:70000]

        self.arr1, self.labelArr1, self.arr2, self.labelArr2, self.arr3, self.labelArr3, self.arr4, self.labelArr4, self.arr5, self.labelArr5 = Split_dataset_by5(x_train,
                                                                                                                                                                  y_train)

        self.arr1_test, self.labelArr1_test, self.arr2_test, self.labelArr2_test, self.arr3_test, self.labelArr3_test, self.arr4_test, self.labelArr4_test, self.arr5_test, self.labelArr5_test = Split_dataset_by5(x_test,
                                                                                                                                                                  y_test)
    ...
    def Create_NewExpert(self):
        ...
        # encoder continoual information
        logit,softmaxValue = Encoder_MNIST_Supervised(self.inputs,self.z, myStr, batch_size=64, reuse=False)
    ...
    def Activate_Expert(self, index):
        ...
        # encoder continoual information
        logit, softmaxValue = Encoder_MNIST_Supervised(self.inputs,self.z, myStr, batch_size=64, reuse=True)
        softmaxValue1 = tf.nn.softmax(logit)
        log_y1 = tf.log(softmaxValue1 + 1e-10)
        discrete1 = my_gumbel_softmax_sample(log_y1, np.arange(10))
    ...
    def build_model(self):
        ...
        z_dim = 150
        # encoder continoual information
        logit,softmaxValue = Encoder_MNIST_Supervised(self.inputs,self.z, "encoder1", batch_size=64, reuse=False)
        softmaxValue1 = tf.nn.softmax(logit)
        log_y1 = tf.log(softmaxValue1 + 1e-10)
        discrete1 = my_gumbel_softmax_sample(log_y1, np.arange(10))
    ...
    def Calculate_FID_Score(self, nextTaskIndex):
        if nextTaskIndex == 0:
            nextTrainX = self.arr1
            nextTrainY = self.labelArr1
        elif nextTaskIndex == 1:
            nextTrainX = self.arr2
            nextTrainY = self.labelArr2
        elif nextTaskIndex == 2:
            nextTrainX = self.arr3
            nextTrainY = self.labelArr3
        elif nextTaskIndex == 3:
            nextTrainX = self.arr4
            nextTrainY = self.labelArr4
        elif nextTaskIndex == 4:
            nextTrainX = self.arr5
            nextTrainY = self.labelArr5
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            for taskIndex in range(taskCount):
                ...
                if taskIndex == 0:
                    currentTrainX = self.arr1
                    currentTrainY = self.labelArr1
                elif taskIndex == 1:
                    currentTrainX = self.arr2
                    currentTrainY = self.labelArr2
                elif taskIndex == 2:
                    currentTrainX = self.arr3
                    currentTrainY = self.labelArr3
                elif taskIndex == 3:
                    currentTrainX = self.arr4
                    currentTrainY = self.labelArr4
                elif taskIndex == 4:
                    currentTrainX = self.arr5
                    currentTrainY = self.labelArr5
