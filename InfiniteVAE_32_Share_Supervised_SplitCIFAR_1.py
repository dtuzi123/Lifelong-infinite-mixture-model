...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        self.x1, self.y1, self.x2, self.y2, self.x3, self.y3, self.x4, self.y4, self.x5, self.y5, self.x_text, self.y_test=Split_CIFAR100()

        self.y1 = to_categorical(self.y1, num_classes=None)
        self.y2 = to_categorical(self.y2, num_classes=None)
        self.y3 = to_categorical(self.y3, num_classes=None)
        self.y4 = to_categorical(self.y4, num_classes=None)
        self.y5 = to_categorical(self.y5, num_classes=None)
        self.y_test = to_categorical(self.y_test, num_classes=None)
    ...
    def Create_NewExpert(self):
        ...
        self.Shared_Encoder = Encoder_SVHN_Shared_Supervised_2_Cifar100(self.inputs,self.z, "encoder_shared", batch_size=64, reuse=True)
        ...
        # encoder continoual information
        logit,softmaxValue = Encoder_SVHN_Specific_Supervised_2_Cifar100(self.Shared_Encoder, myStr, batch_size=64, reuse=False)
    ...
    def Activate_Expert(self, index):
        ...
        # encoder continoual information
        logit, softmaxValue = Encoder_SVHN_Specific_Supervised_2_Cifar100(self.Shared_Encoder, myStr, batch_size=64, reuse=True)
        softmaxValue1 = tf.nn.softmax(logit)
        log_y1 = tf.log(softmaxValue1 + 1e-10)
        discrete1 = my_gumbel_softmax_sample(log_y1, np.arange(10))
    ...
    def build_model(self):
        ...
        z_dim = 150
        # encoder continoual information
        self.Shared_Encoder = Encoder_SVHN_Shared_Supervised_2_Cifar100(self.inputs,self.z, "encoder_shared", batch_size=64, reuse=False)
        logit,softmaxValue = Encoder_SVHN_Specific_Supervised_2_Cifar100(self.Shared_Encoder, "encoder1", batch_size=64, reuse=False)
        softmaxValue1 = tf.nn.softmax(logit)
        log_y1 = tf.log(softmaxValue1 + 1e-10)
        discrete1 = my_gumbel_softmax_sample(log_y1, np.arange(10))
        ...
        with tf.Session(config=config) as sess:
            ...
            for taskIndex in range(taskCount):
                ...
                if taskIndex == 0:
                    ...
                    currentTrainX = self.x1
                    currentTrainY = self.y1
                elif taskIndex == 2:
                    currentTrainX = self.x2
                    currentTrainY = self.y2
                elif taskIndex == 3:
                    currentTrainX = self.x3
                    currentTrainY = self.y3
                elif taskIndex == 4:
                    currentTrainX = self.x4
                    currentTrainY = self.y4
                elif taskIndex == 5:
                    currentTrainX = self.x5
                    currentTrainY = self.y5
            ...
            #Testing phase
            x1_, y1_, x2_, y2_, x3_, y3_, x4_, y4_, x5_, y5_ = Split_CIFAR100_ReturnTesting()

            index1 = self.Select_Expert_ByData2(x1_,y1_)
            acc1 = self.Calculate_Accuracy_ByIndex(x1_,y1_, index1)

            print("index:")
            print(index1)

            index2 = self.Select_Expert_ByData2(x2_,y2_)
            acc2 = self.Calculate_Accuracy_ByIndex(x2_,y2_, index2)

            print("index:")
            print(index2)

            index3 = self.Select_Expert_ByData2(x3_,y3_)
            acc3 = self.Calculate_Accuracy_ByIndex(x3_,y3_, index3)

            print("index:")
            print(index3)

            index4 = self.Select_Expert_ByData2(x4_,y4_)
            acc4 = self.Calculate_Accuracy_ByIndex(x4_,y4_, index4)

            print("index:")
            print(index4)

            index5 = self.Select_Expert_ByData2(x5_,y5_)
            acc5 = self.Calculate_Accuracy_ByIndex(x5_,y5_, index5)

            print("index:")
            print(index5)

            index = self.Select_Expert_ByData2(self.cifar_test_x,self.cifar_test_label)
            cifarError = self.Calculate_Accuracy_ByIndex(self.cifar_test_x,self.cifar_test_label, index)

            print("index:")
            print(index)

            sum1 =cifarError+acc1+acc2+acc3+acc4+acc5
            sum1 = sum1 / 6.0
